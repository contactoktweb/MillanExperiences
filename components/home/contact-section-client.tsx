...
const translations = {
  en: {
    ...
    description: "Tell us how you envision your next escape. We will take care of the rest, ensuring the flawless execution of your itinerary so you can simply enjoy the experience at your own pace.",
    thankYou: "Thank you.",
    successMsg: "Your request has reached our concierge. We will be in touch shortly with a tailored proposal. Everything is handled from here.",
    step1: "Step 01 — The Journey",
    ...
    whatsappFallback: "Prefer WhatsApp? Speak with our concierge."
  },
  es: {
    services: [
      "Lanchas y Yates",
    ...
    description: "Cuéntanos cómo imaginas tu próxima escapada. Nosotros nos encargaremos del resto, asegurando la ejecución impecable de tu itinerario para que puedas disfrutar de la experiencia a tu propio ritmo.",
    thankYou: "Gracias.",
    successMsg: "Tu solicitud ha llegado a nuestro concierge. Nos pondremos en contacto pronto con una propuesta a medida. Nos encargamos de todo a partir de aquí.",
    step1: "Paso 01 — El Viaje",
    ...
    whatsappFallback: "¿Prefieres WhatsApp? Habla con nuestro concierge."
  }
}
...
export function ContactSectionClient({ whatsappUrl, locale = "en" }: { whatsappUrl?: string, locale?: "en" | "es" }) {
  ...
  const validateStep2 = () => {
    ...
    if (!form.phone.trim() || form.phone.trim() === "+57") next.phone = t.required
    if (!/^\S+@\S+\.\S+$/.test(form.email)) next.email = t.required
    setErrors(next)
    return Object.keys(next).length === 0
  }
    ...
    <section id="contact" className="bg-[var(--color-blue-gray)] py-24 md:py-40">
      <div className="mx-auto grid max-w-[1440px] grid-cols-1 gap-0 px-0 lg:grid-cols-2">
        ...
        <div className="bg-[var(--color-deep-sea)] px-6 py-16 text-[var(--color-warm-white)] sm:px-10 md:px-14 md:py-20">
          <p className="eyebrow text-[var(--color-sand)]">{t.contact}</p>
          ...
          {submitted ? (
            ...
            >
              ...
              <h3 className="mt-6 font-serif text-2xl">{t.thankYou}</h3>
                ...
                {step === 1 ? (
                  <div className="grid grid-cols-1 gap-7 sm:grid-cols-2">
                    ...
                    <div className={fieldGap}>
                      <label htmlFor="startDate" className={labelBase}>{t.startDate}</label>
                      <input
                        id="startDate"
                        type="date"
                    ...
                    <div className={fieldGap}>
                      <label htmlFor="endDate" className={labelBase}>{t.endDate}</label>
                      <input
                        id="endDate"
                        type="date"
                        value={form.endDate}
                  ...
                  <div className="grid grid-cols-1 gap-7 sm:grid-cols-2">
                    <div className={fieldGap}>
                      <label htmlFor="firstName" className={labelBase}>{t.name}</label>
                      <input
                        id="firstName"
                        type="text"
                    ...
                    <div className={fieldGap}>
                      <label htmlFor="lastName" className={labelBase}>{t.lastName}</label>
                      <input
                        id="lastName"
                        type="text"
                        value={form.lastName}
